...
class UserManager(models.Manager):
    def register_validator(self, postData):
        ...
        if len(postData['first_name']) == 0 and len(postData['last_name']) == 0 and len(postData['email']) == 0 and len(postData['password']) == 0 and len(postData["password_confirm"]) == 0:
...
class Location(models.Model):
    ...
    year_round = models.CharField(max_length=5)
    logo = models.TextField()
    fav_by = models.ManyToManyField(User, related_name="favorites")
